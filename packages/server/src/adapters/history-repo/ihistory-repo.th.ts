  ...
  describe("find", () => {
    it("正常に検索出来るか", async () => {
      await $isolate(async (repo) => {
        ...
        expect(await repo.find({}, 3)).toEqual([history4, history2, history1]);
        ...
        expect(await repo.find({ topic: ["topic2"] }, 100)).toEqual([history4]);
        ...
        expect(await repo.find({ id: ["history2", "a"] }, 100)).toEqual([
          history2,
        ]);
