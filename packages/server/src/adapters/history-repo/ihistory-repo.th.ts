...
import { AtError, History, IHistoryRepo, emptyHistoryRepoQuery } from "../../";
  ...
  describe("find", () => {
    it("正常に検索出来るか", async () => {
      await $isolate(async (repo) => {
        ...
        const history2 = history.copy({
          id: "history2",
          topic: "topic1",
          date: new Date(80),
        });
        const history3 = history.copy({
          id: "history3",
          topic: "topic1",
          date: new Date(30),
        });
        const history4 = history.copy({
          id: "history4",
          topic: "topic2",
          date: new Date(90),
        });
        ...
        ).toEqual([history4, history2, history1, history3]);

        expect(await repo.find({ ...emptyHistoryRepoQuery }, 3)).toEqual([
          history4,
          history2,
          history1,
        ]);
        ...
        // topic
        expect(
          await repo.find({ ...emptyHistoryRepoQuery, topic: ["topic1"] }, 100)
        ).toEqual([history2, history1, history3]);

        expect(
          await repo.find({ ...emptyHistoryRepoQuery, topic: ["topic2"] }, 100)
        ).toEqual([history4]);

        expect(
          await repo.find({ ...emptyHistoryRepoQuery, topic: ["topic3"] }, 100)
        ).toEqual([]);

        expect(
          await repo.find({ ...emptyHistoryRepoQuery, topic: [] }, 100)
        ).toEqual([]);

        expect(
          await repo.find(
            { ...emptyHistoryRepoQuery, topic: ["topic1", "topic2"] },
            100
          )
        ).toEqual([history4, history2, history1, history3]);
        ...
        // id
        expect(
          await repo.find({ ...emptyHistoryRepoQuery, id: [] }, 100)
        ).toEqual([]);
        expect(
          await repo.find(
            { ...emptyHistoryRepoQuery, id: ["history2", "a"] },
            100
          )
        ).toEqual([history2]);
        expect(
          await repo.find(
            { ...emptyHistoryRepoQuery, id: ["history2", "history4"] },
            100
          )
